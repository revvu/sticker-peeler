...
import { encodeStateKey, loadEncoderWeights } from "./cubeEncoder.js";
...
export function initJepaEncoder(url = "assets/jepa/encoder_weights.json") {
  if (!initPromise) {
    initPromise = loadEncoderWeights(url)
      ...
      .catch((error) => {
        console.warn("JEPA encoder weights unavailable:", error);
        encoderWeights = null;
