...
DEFAULT_OUTPUT = REPO_ROOT / "assets" / "supervised-distance" / "encoder_weights.json"
...
def export_encoder(checkpoint_path: Path, output_path: Path) -> None:
    ...
    payload = {
        ...
        "sticker_embed_dim": config["sticker_embed_dim"],
        "hidden_dim": config["hidden_dim"],
        "sticker_embed": tensor_to_list(model.encoder.sticker_embed.weight),
        "mlp": mlp_layers,
        "distance_head": {
            "weight": tensor_to_list(model.head.weight),
            "bias": tensor_to_list(model.head.bias),
        },
    ...
    with torch.no_grad():
        distance = model(solved_indices).item()

    print(f"Exported supervised distance model from {checkpoint_path} to {output_path}")
    print(f"Checkpoint epoch: {checkpoint.get('epoch')}")
    print(f"Predicted distance at solved: {distance:.4f}")
