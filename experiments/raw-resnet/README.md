...
Distance-from-solved classifier over a one-hot 9×6 cube image. Each pixel is one of six sticker colors; the model outputs a 27-way softmax over scramble depths 0–26.
...
- Custom ResNet with **6 residual blocks** (`models/raw_resnet.py`).
...
```sh
cd experiments/raw-resnet
python train.py --run-name raw-resnet-v1
```
...
```sh
tensorboard --logdir experiments/raw-resnet/runs
```
