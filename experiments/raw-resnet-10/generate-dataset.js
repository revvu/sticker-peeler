...
const TRAIN_SAMPLES_TARGET = 1_004_000;
const VAL_TEST_PATH_RATIO = 0.1;
const SEED = 42;
...
const trainPathCount = Math.floor(TRAIN_SAMPLES_TARGET / SAMPLES_PER_PATH);
const trainSamples = trainPathCount * SAMPLES_PER_PATH;
const valPathCount = Math.floor(trainPathCount * VAL_TEST_PATH_RATIO);
const testPathCount = valPathCount;
...
console.log(
  `Target train samples: ${TRAIN_SAMPLES_TARGET.toLocaleString()} ` +
    `(using ${trainSamples.toLocaleString()} = ${trainPathCount.toLocaleString()} paths × ${SAMPLES_PER_PATH})`
);
...
const stats = {
  ...
  trainSamplesTarget: TRAIN_SAMPLES_TARGET,
  train: {
